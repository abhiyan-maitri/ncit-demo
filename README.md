...
| 3 | OCR a 1000-page PDF       | up to you |
...
Verify: `sam --version` should print `SAM CLI, version 1.x.x`
...
- You describe your **infrastructure** (Lambda, S3, SQS, DynamoDB, IAM, etc.)
...
```
ncit-demo/
├── template.yaml         (SAM template. Add scenarios 1 to 4 here.)
...
1. Add the AWS resources (Lambda, queue, bucket, etc.) under `Resources:` in `template.yaml`.
...
npm run build            # esbuild compiles to .aws-sam/build/
...
| `npm run build`         | Compile TypeScript to CommonJS bundles via esbuild |
...
- S3 bucket with an ObjectCreated notification feeding SQS
...
### Scenario 3: OCR a 1000-page PDF

See the **Hands-on problem** at the end of this README. The design
...
| Deploy hangs at `CREATE_IN_PROGRESS` for an S3 bucket | A QueuePolicy is missing, so S3 can't publish to your SQS queue. Add it. |
...
> **OCR a 1000-page PDF, end-to-end, on AWS.**

Users drop a PDF into S3. The system must produce extracted text
back in S3, automatically and reliably.

**Inputs / outputs**

```
input    s3://<your-bucket>/incoming/<name>.pdf      (up to 1000 pages)
output   s3://<your-bucket>/extracted/<name>/...     (extracted text)
```

**Requirements**

- Trigger **automatically** when a PDF lands in `incoming/`.
- Must work for PDFs that take **longer than 15 minutes** to process.
- A bad or corrupt PDF must **not block good ones**.
- Same PDF uploaded twice must **not** corrupt the output (idempotency).
- Every async hop needs a **DLQ** so failures are inspectable, not lost.

Design it on paper first. Defend your choices. Then build it.
